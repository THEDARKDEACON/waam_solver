...
def bead_reinforcement_width_mm(twin: "WAAMTwin", g: "WAAMGrid") -> float:
    """Width of solidified metal at or above the substrate (mm)."""
    flags = g.flags.to_numpy()
    nz_ref = twin.nz_solid
    solid = flags[:, :, nz_ref:] == g.FLAG_SOLID
    if not solid.any():
        return 0.0
    y_idx = solid.any(axis=(0, 2)).nonzero()[0]
    if y_idx.size == 0:
        return 0.0
    return float((int(y_idx.max()) - int(y_idx.min()) + 1) * g.dx * 1000.0)
